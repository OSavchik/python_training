...
def test_check_contact_on_form_and_DB(app, db):
    ...
    count_elements = form_contacts.__len__()
    element=0
    while element < count_elements:
        ...
        assert re.sub(r'\s+', '', contact_from_form.all_email) == contact_from_DB.all_email
        s1 = re.sub(r'\s+', '', contact_from_form.all_phones_from_home_page).replace(' ', '')
        print(s1)
        s2 = re.sub(r'\s+', '', contact_from_DB.all_phones_from_home_page).replace(' ', '')
